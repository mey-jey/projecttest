...
class CalendarCore extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PageView.builder(
      ...
      itemCount: _getPageCount(calendarFormat, firstDay, lastDay, isJalali),
      itemBuilder: (context, index) {
        final baseDay = _getBaseDay(calendarFormat, index, isJalali);
        ...
        return CalendarPage(
          ...
          dayBuilder: (context, day) {
            ...
            if (previousFocusedDay == null || previousIndex == null) {
              baseDay = _getBaseDay(calendarFormat, index, isJalali);
            } else {
              baseDay = _getFocusedDay(
                  calendarFormat, previousFocusedDay, index, isJalali);
            }
  ...
  int _getPageCount(
      CalendarFormat format, DateTime first, DateTime last, bool isJalali) {
    switch (format) {
      case CalendarFormat.month:
        return PrivateUtils.getMonthCount(first, last, isJalali) + 1;
  ...
  DateTime _getFocusedDay(CalendarFormat format, DateTime prevFocusedDay,
      int pageIndex, bool isJalali) {
    ...
    DateTime day;
    if (isJalali) {
      var JPrevFocusedDay = prevFocusedDay.toJalali();
      switch (format) {
        case CalendarFormat.month:
          day = _ToNormalJalali(
                  JPrevFocusedDay.year, (JPrevFocusedDay.month + pageDif))
              .toUtcDateTime();
          break;
        case CalendarFormat.twoWeeks:
          day = _ToNormalJalali(JPrevFocusedDay.year, JPrevFocusedDay.month,
                  JPrevFocusedDay.day + pageDif * 14)
              .toUtcDateTime();
          break;
        case CalendarFormat.week:
          day = _ToNormalJalali(JPrevFocusedDay.year, JPrevFocusedDay.month,
                  JPrevFocusedDay.day + pageDif * 7)
              .toUtcDateTime();
          break;
      }
    } else {
      switch (format) {
        case CalendarFormat.month:
          day =
              DateTime.utc(prevFocusedDay.year, prevFocusedDay.month + pageDif);
          break;
        case CalendarFormat.twoWeeks:
  ...
  DateTime _getBaseDay(CalendarFormat format, int pageIndex, bool isJalali) {
    DateTime day;

    if (isJalali) {
      var jFirstDay = firstDay.toJalali();
      switch (format) {
        case CalendarFormat.month:
          day = _ToNormalJalali(jFirstDay.year, jFirstDay.month + pageIndex)
              .toUtcDateTime();
          break;
        case CalendarFormat.twoWeeks:
          day = _ToNormalJalali(jFirstDay.year, jFirstDay.month,
                  jFirstDay.day + pageIndex * 14)
              .toUtcDateTime();
          break;
        case CalendarFormat.week:
          day = _ToNormalJalali(jFirstDay.year, jFirstDay.month,
                  jFirstDay.day + pageIndex * 7)
              .toUtcDateTime();
          break;
      }
    } else {
  ...
  Jalali _ToNormalJalali(int year, [int month = 1, int day = 0]) {
    if (month == 0) {
      var jalali = Jalali(year - 1, 12);
      jalali = jalali.addDays(day);
      return jalali;
    } else if (month < 13) {
      var jalali = Jalali(year, month);
      jalali = jalali.addDays(day);
      return jalali;
    } else {
      var jalali = Jalali(year);
      jalali = jalali.addMonths(month - 1);
      jalali = jalali.addDays(day);
      return jalali;
    }
  }
